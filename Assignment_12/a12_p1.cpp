...
Hexagon::Hexagon() : RegularPolygon("Default Hexagon", 5, 8, 9) {
    colour = "burgundy";
    side = 7;
}
// Default Constructor

Hexagon::Hexagon(const string& n, double nx, double ny, int nl, double newside, const string& newcolour) : RegularPolygon(n, nx, ny, nl) {
    side = newside;
    colour = newcolour;
}
// Constructor for 6 parameters

Hexagon::Hexagon(const Hexagon& copy) : RegularPolygon(copy.getName(), copy.getx(), copy.gety(), copy.getEdgesNumber()) {
    side = copy.side;
    colour = copy.colour;
}
// Copy Constructor
...
const string& Hexagon::getcolour() {
    return colour;
}
// Getter Methods

double Hexagon::area() {
    return (pow(getside(), 2) * (3/2) * sqrt(3) );
}
